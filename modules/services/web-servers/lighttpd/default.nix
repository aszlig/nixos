...
in {
  ...
  config = mkIf config.services.lighttpd.enable {
    ...
    environment.systemPackages = [lighttpd];

    jobs.lighttpd = {
      description = "Lighttpd webserver";
      buildHook = "${lighttpd}/sbin/lighttpd -t -f ${lightyCfgFile}";
      ...
      # startOn optional depends?
      daemonType = "daemon";
      exec = "${lighttpd}/sbin/lighttpd -f ${lightyCfgFile}";
    };
  };
}
